...
import com.vxx0.aso.ore_no_yome.adapters.NoticeListAdapter
import com.vxx0.aso.ore_no_yome.adapters.NoticeListItem
...
class MainFragment : Fragment(), SwipeRefreshLayout.OnRefreshListener {
    ...
    private fun switchListView() {
        when (action) {
            "home" -> {
                setHomeList(root_layout)
            }
            "favorite" -> {
                setFavoriteList(root_layout)
            }
            "notice" -> {
                setNoticeList(root_layout)
            }
            else -> {
                setHomeList(root_layout)
            }
    ...
    private fun setHomeList(view: View) {
        ApiGetTask {
            if (it == null) {
                ...
                when (it.getString("status")) {
                    "S00" -> {
                        val timelineData = it.getJSONArray("data")
        ...
        }.execute(ApiParams("post/get-timeline"))
    }

    private fun setFavoriteList(view: View) {
    ...
    private fun setNoticeList(view: View) {
        val pref = activity!!.getSharedPreferences("data", MODE_PRIVATE)
        val userToken = pref.getString("token", "") as String
        ApiGetTask {
            if (it == null) {
                ...
                when (it.getString("status")) {
                    "S00" -> {
                        val timelineData = it.getJSONArray("data")
                        val list = ArrayList<NoticeListItem>()
                        for (i in 0 until timelineData.length()) {
                            val noticeListItem = NoticeListItem()
                            val item = timelineData.getJSONObject(i)
                            noticeListItem.id = i.toLong()
                            noticeListItem.userId = item.getString("user_id")
                            noticeListItem.userName = item.getString("user_name")
                            noticeListItem.icon = item.getString("icon")
                            noticeListItem.content = item.getString("content")
                            list.add(noticeListItem)
                        }
                        val listView = timeline
                        val noticeListAdapter = NoticeListAdapter(activity!!)
                        noticeListAdapter.setNoticeList(list)
                        noticeListAdapter.notifyDataSetChanged()
                        listView.adapter = noticeListAdapter
                        listView.setOnItemClickListener { adapterView, _, i, _ ->
                            val item = adapterView.getItemAtPosition(i) as NoticeListItem
                            /*
                            val intent = Intent(activity!!, MainPostDetailActivity::class.java).apply {
                                putExtra("post_id", item)
                            }
                            startActivity(intent)
                            */
                        }
            ...
            mSwipeRefreshLayout.isRefreshing = false
        }.execute(ApiParams("user/get-notice", hashMapOf("token" to userToken)))
    }
}
